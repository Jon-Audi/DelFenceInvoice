...
export default function InvoicesPage() {
  ...
  useEffect(() => {
    ...
    if (pendingInvoiceRaw) {
      ...
      try {
        ...
        const invoiceTotal = estimateToConvert.lineItems.reduce((sum, item) => sum + item.total, 0);
        const newInvoiceData: InvoiceFormData = {
          // id field is optional in InvoiceFormData and will be generated by InvoiceDialog if not present
          ...
          // Payment fields should be initialized as undefined for a new invoice from estimate
    ...
    const unsubscribe = onSnapshot(collection(db, 'invoices'), (snapshot) => {
      ...
      snapshot.forEach((docSnap) => {
        ...
        fetchedInvoices.push({
             ...data as Omit<Invoice, 'id' | 'total' | 'amountPaid' | 'balanceDue'>, // Assume these are on data
             id: docSnap.id,
             // Ensure these fields have default values if not present in Firestore
             total: data.total || 0,
  ...
  const handleSaveInvoice = async (invoiceToSave: Invoice) => {
    ...
    // Ensure payment fields are initialized if not present
    const dataToSave = {
        ...invoiceData,
